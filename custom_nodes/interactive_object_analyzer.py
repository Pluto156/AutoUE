# ============================================================
# File Name: interactive_object_analyzer.py
# Purpose:
#   Analyze the outputs of KeyElementExtractor and ModuleCodeGenerator,
#   infer the interaction logic between each model and module,
#   determine whether a battle system is required,
#   and identify whether the object is a movable AI enemy unit
#   that uses a navigation system.
# ============================================================
...
# Prompt Definition
# ============================================================
INTERACTIVE_OBJECT_ANALYZER_PROMPT = """
...
# ============================================================
# Input Construction Function
# ============================================================
def GetInput(node: BaseLLMNode, state: GraphState, full_input: str):
    key_elements = state.llm_outputs.get("KeyElementExtractor", "")
    module_outputs = state.llm_outputs.get("ModuleCodeGenerator", "")
    return (
        f"KeyElementExtractor output:\n{key_elements}\n\n"
        f"ModuleCodeGenerator output:\n{module_outputs}\n"
    )
...
def save_json(state: GraphState, output):
    ...
    save_path = os.path.join(os.getenv("UE_Save"), "PCG", "InteractiveObjectAnalyzer.json")
    ...
    try:
        ...
        print(f"[ERROR] Failed to save InteractiveObjectAnalyzer output: {e}")
        print("[DEBUG] Original output:\n", output)


def create_interactive_object_analyzer() -> BaseLLMNode:
    return BaseLLMNode(
        name="InteractiveObjectAnalyzer",
        prompt=INTERACTIVE_OBJECT_ANALYZER_PROMPT,
